...
| `docs/SLEEP_INTERIOR_AROUSALS.md` | **Interior arousal detection — BUILT 2026-08-15. Second, lower tail-sum cut for interior-only arousals (existing 345 cut fires ONLY at night edges). §1b's fix is 🟢 CONFIRMED on a real re-staged night (2026-08-18), but `arousalIntensityCut = 200` is 🔴 REFUTED as a global constant — it gave 3 awakenings one night and 22 the next. Read §1b's "Measured 2026-08-18" before trusting §4's "200 chosen"; open question in `docs/PENDING_VALIDATION.md` → `sleep-arousal-cut-refit`** |
...
| `docs/HEALTH_ALERTS_SPO2.md` | **Low-SpO2 alert — plan of record. BUILT 2026-08-18: burst-artifact rejection (a healthy reading seconds away refutes a candidate) + first-sighting gate (a reading fires at most once, on the pass that first sees it), fixing a real 12h-late false-positive notification. Read §1–2 for the incident, §5 for how `desktop/device_alert_audit.py --not-before` verifies it. Real-world confirmation still pending — see `docs/PENDING_VALIDATION.md` → `spo2-burst-fix-real-world`** |
