...
| `docs/SLEEP_INTERIOR_AROUSALS.md` | **THE FIX for brief awakenings — plan of record (PROPOSED, not built).** 🟢 Step-1 measured: the intensity-tail fallback is ALREADY engaged; the sole blocker is the single `motionIntensityActiveCut = 345`, which fires only at the night's EDGES (22 pre-onset + 2 post-wake, 0 interior). Adds a second, lower interior-only cut. Read §2's two safety properties |
